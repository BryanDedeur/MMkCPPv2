...
def fleury_walk(graph, start=1, circuit=True):
    """
...
def eularian_path(graph, start=1, circuit=True):
    """
    ...
    """
    print("Attempting fleury walk: [", end='')
...
def find_minimum_path_set(pair_sets, pair_solutions):
    ...
    for pair_set in pair_sets:
        ...
        if set_cost < min_cost:
            ...
            min_cost = set_cost
            print("New min cost route found: " + str(min_cost))
            min_route = [pair_solutions[pair][1] for pair in pair_set]
            print("Route: {}".format(min_route))
